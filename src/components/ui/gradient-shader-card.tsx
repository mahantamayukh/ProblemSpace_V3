...
const GrainyGradient = forwardRef<any, GrainyGradientProps>(({ ripples = [], onTimeUpdate, isDarkMode = false }, ref) => {
  ...
  // Update dark mode uniform when props change
  useEffect(() => {
  ...
  useFrame((state) => {
    if (mesh.current) {
      ...
      for (let i = 0; i < Math.min(activeRipples.length, 10); i++) { // Max 10 ripples
        ...
        // Convert to normalized coordinates (-1 to 1) based on actual screen size instead of hardcoded 800x600
...
const fragmentShader = `
  ...
  vec3 multiColorGradient(float t) {
    ...
    if (isDarkMode) {
      // Inspired by the dark UI with neon green/lime accents
      colors[0] = hexToRgb(204.0, 255.0, 0.0);   // Neon Yellow-Green (#CCFF00)
      colors[1] = hexToRgb(163.0, 230.0, 53.0);  // Lime Green (#A3E635)
      colors[2] = hexToRgb(77.0, 124.0, 15.0);   // Darker Forest Green (#4D7C0F)
      colors[3] = hexToRgb(20.0, 25.0, 18.0);    // Very Dark Green-Tinted Grey
      colors[4] = hexToRgb(10.0, 10.0, 10.0);    // Almost Black
      colors[5] = hexToRgb(4.0, 4.0, 4.0);       // Pitch Black
      colors[6] = hexToRgb(0.0, 0.0, 0.0);       // Pitch Black
    } else {
  ...
  void mainImage(out vec4 O, in vec2 I) {
    // Multiply by 0.85 to zoom IN slightly without causing pixelation blur
    vec2 uv = ((I - 0.5 * iResolution.xy) / iResolution.y) * 0.85;
    ...
    float archFactor = 1.0 - distanceFromCenter * 0.2; // Flattened arch for wider screens

    // Primary wave with arch
    float wave1 = sin(warpedUv.x * 2.0 + phase1) * 0.5 * archFactor;
    // Secondary wave with different frequency and arch
    float wave2 = sin(warpedUv.x * 3.5 - phase2) * 0.3 * archFactor;
    // Tertiary wave that creates vertical arching
    float wave3 = sin(warpedUv.y * 3.0 + phase1 * 0.7) * 0.15;

    // Add a parabolic arch effect (flattened for wide screens)
    float parabolicArch = -pow(warpedUv.x * 0.5, 2.0) * 0.05;
